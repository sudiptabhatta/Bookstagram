...
export default function Profile() {
  ...
  const [dataFromChildUser, setDataFromChildUser] = useState(false);
  ...
  const handleDataFromChildUser = (data) => {
    setDataFromChildUser(data);
  }

  return (
    <div>
      <NavbarLayout user={user} setUser={setUser} />
      <User user={user} setUser={setUser} sendDataToParentProfile={handleDataFromChildUser} />
      {dataFromChildUser === false && loggedinUsername !== user.username ? (
        <>
          <Container className='mt-5 text-center bg-slate-50 p-3'>
            <Row>
              <Col>
                <h5>Follow to View All Books in This Profile.</h5>
              </Col>
            </Row>
          </Container>
        </>
          ...
          {user.book_count > 0 ? (
            ...
            <>
              <Container>
                <Row>
                  <Col md={3}></Col>
                  <Col><p align="center">No Books Found</p></Col>
                  <Col md={3}></Col>
                </Row>
              </Container>
            </>
